<x-app-layout>
    ...
    <div class="row">
        <div class="col-md-4 offset-4 mt-5 p-0">
            <form action="{{ route('houses.update',$product->id) }}" method="POST">
                @csrf
                @method('PUT')

                <h3 class="mb-3 bg-dark text-white rounded-top p-3">Edit house: {{ $product->id }}</h3>
                <div class="mb-3">
                    <label for="house_name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="house_name" name="house_name" value="{{ $product->name }}">
                </div>
                @error('house_name')
