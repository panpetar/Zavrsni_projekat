<x-app-layout>
    ...
    <div class="row">
        <div class="col-md-4 offset-4 mt-5 p-0">
            <form action="{{ route('snowmen.store') }}" method="POST">
                @csrf

                <h3 class="mb-3 bg-dark text-white rounded-top p-3">Create new snowmen:</h3>
                <div class="mb-3">
                    <label for="snowmen_name" class="form-label">Name</label>
                    <input type="text" class="form-control" id="snowmen_name" name="snowmen_name" value="{{ old('snowmen_name') }}">
                </div>
                @error('snowmen_name')
